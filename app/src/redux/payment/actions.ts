          ...
          setOrderErrors([
            {
              code: 'timeout',
              message:
                'The request could not be executed within the allotted time. Please retry later.',
            },
          ])
        );
    ...
    try {
        ...
        await dispatch(
          sendPaymentStep({
            ...
            paymentFunction: async () =>
              DexProxy.checkAndApproveAllowance(dexTrade, ethManager.provider),
          })
        );
      ...
      await dispatch(
        sendPaymentStep({
          ...
          paymentFunction: async () =>
            ethManager.send(bityDepositAddress, bityInputAmount).then(tx => tx.hash),
        })
      );
