...
export const initBox = () =>
  async function (dispatch, getState) {
    ...
    try {
      ...
      if (!getBoxLoading(state) || !getETHManager(getState())) {
        // cancelled or User disconnected while fetching box
        return;
      }
...
export const cancelInitBox = () =>
  async function (dispatch) {
    log('Box opening cancelled');
    dispatch(resetBox());
  };

export const initBoxIfLoggedIn = () =>
  async function (dispatch, getState) {
    const ethManager = getETHManager(getState());
