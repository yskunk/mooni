...
class Bity {
  ...
  async estimate(tradeRequest: TradeRequest): Promise<BityTrade> {
    ...
    try {
      ...
      if (
        ...
        throw new BityOrderError('bity_amount_too_low', [
      ...
      if (error?.response?.data?.errors) {
        throw new BityOrderError('api_error', error.response.data.errors);
      } else {
        throw error;
      }
    }
  ...
  async createOrder(
    ...
    if (tradeRequest.tradeExact === TradeExact.INPUT)
      body.input.amount = String(tradeRequest.amount);
    ...
    try {
      ...
      if (!bityOrderResponse.input) {
        throw new BityOrderError('api_error', [
