...
function defaultRateState(initialTradeRequest: TradeRequest): RateState {
  let values = {
    ...
    inputAmount:
      initialTradeRequest.tradeExact === TradeExact.INPUT ? initialTradeRequest.amount : null,
    outputAmount:
      initialTradeRequest.tradeExact === TradeExact.OUTPUT ? initialTradeRequest.amount : null,
...
const rateStateReducer: RateStateReducer = (state: RateState, action: RateStateAction) => {
  switch (action.type) {
    ...
    case 'setCurrency': {
      ...
      const currencyObjectKey =
        tradeExact === TradeExact.INPUT ? 'inputCurrencyObject' : 'outputCurrencyObject';
      ...
      return {
        ...
        rateForm: {
          ...
          values: {
            ...
            [state.rateForm.values.tradeExact === TradeExact.INPUT
              ? 'outputAmount'
              : 'inputAmount']: null,
          },
        },
    ...
    case 'setEstimation': {
      ...
      const amount =
        tradeExact === TradeExact.INPUT
          ? multiTradeEstimation.outputAmount
          : multiTradeEstimation.inputAmount;
...
const estimate = async (tradeRequest: TradeRequest, _nonce: number): Promise<RateStateAction> => {
  ...
  if (
    ...
    new BN(multiTradeEstimation.outputAmount).gt(outputLimit)
  ) {
    return {
      type: 'setError',
      payload: {
