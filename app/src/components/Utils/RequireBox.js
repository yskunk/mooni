...
function BoxLoading() {
  ...
  return (
    <Box
      ...
      <Box fontSize={14} width={1} textAlign="center" mt={1}>
        Loading 3Box user data...
      </Box>
      <Box fontSize={14} width={1} textAlign="center">
        You may need to sign messages in your wallet.
      </Box>
      <Box display="flex" justifyContent="flex" mb={2}>
        <LoadingRing />
      </Box>
      <Button mode="negative" onClick={cancel}>
        Cancel
      </Button>
    </Box>
  );
...
export function BoxModal({ visible, onClose }) {
  ...
  return (
    <Modal visible={visible} closeButton={false} width={300} padding={16}>
      <Box
        display="flex"
        justifyContent="center"
        alignItems="center"
        ...
        width={1}
        p={2}>
        <Box fontSize={14} width={1} textAlign="center" mb={2}>
          To save your bank account informations, you need to connect to 3Box.
        </Box>
        <Box display="flex" justifyContent="space-between" width={1}>
          <Button mode="negative" wide onClick={onClose}>
            Cancel
          </Button>
          <Box width={10} />
          <Button mode="positive" wide onClick={enableBox}>
            Connect
          </Button>
        </Box>
      </Box>
