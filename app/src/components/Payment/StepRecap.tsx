...
function StepRecap({ onComplete, onStartOver }) {
  ...
  if (orderErrors) {
    return <OrderError orderErrors={orderErrors} onStartOver={onStartOver} />;
  }

  if (!multiTrade) {
    return <Loader text="Creating order ..." />;
  }
  ...
  return (
    <Box width={1}>
      ...
      <Box py={2} display="flex" justifyContent="center">
        ...
        <label>
          I agree with the{' '}
          <Link onClick={() => dispatch(setInfoPanel('terms'))}>terms of service</Link>
        </label>
      </Box>
      <RoundButton
        mode="strong"
