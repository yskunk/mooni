...
function RateForm({ onSubmit = () => null, initialTradeRequest }: RateFormParams) {
  ...
  const {
    ...
    onChangeAmount,
    onChangeCurrency,
  } = useRate(initialTradeRequest);
  const { approvalState, approveAllowance } = useApprovalForMultiTradeEstimation(
    multiTradeEstimation
  );
  const { balanceLoading, insufficientBalance, maxAmount } = useTradeBalance(
    tradeRequest,
    multiTradeEstimation
  );
  ...
  if (walletStatus === WalletStatus.CONNECTED) {
    if (errors && errors.zeroAmount) {
      ...
      button = (
        <RoundButton mode="strong" onClick={submit} wide icon={<IconRefresh />} label="Exchange" />
      );
    }
    ...
    button = (
      <RoundButton disabled wide icon={<LoadingRing />} display="all" label="Connecting..." />
    );
  }
        ...
        <Flex justify="center" mt={errors.zeroAmount ? 0 : 4}>
          {Object.entries(errors).map(([key, _]) => (
            <InvalidMessage key={key}>
              {key === 'lowAmount' &&
                `Minimum amount is ${errors[key]} ${rateForm.values.outputCurrency}`}
              {key === 'highAmount' &&
                `Maximum amount is ${numberWithCommas(
                  dailyLimits[rateForm.values.outputCurrency]
