    ...
    async (req: NowRequest, res: NowResponse, token: Token): Promise<NowResponse | void> => {
      ...
      if (!compareAddresses(multiTradeRequest.ethInfo.fromAddress, token.claim.iss)) {
        throw new APIError(
          400,
          'different-addresses',
          'different ethereum address used for order and authentication'
        );
      }
      ...
      if (multiTradeRequest.referralId) {
        ...
        if (multiTradeRequest.referralId === user.referralId) {
          throw new APIError(
            400,
            'self-referral',
            'you cannot create an order by referring yourself'
          );
        }
      ...
      await bityInstance.initializeAuth(
        config.private.bityClientId,
        config.private.bityClientSecret
      );
